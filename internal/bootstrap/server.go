...
func NewServer(configPath string) (*iris.Application, *config.Config, error) {
	...
	corsHandler := cors.New().
		AllowOrigin("*").
		AllowHeaders("Origin", "Content-Type", "Accept", "Authorization", "X-Requested-With", "X-Visitor-Id", "X-Widget-App-Id").
		MaxAge(600).
		ExposeHeaders("Content-Length", "Content-Type", "Authorization", "X-Visitor-Id", "X-Widget-App-Id").
		Handler()
...
func addRouter(app *iris.Application, cfg *config.Config) {
	...
	mvc.Configure(app.Party("/api/console", AuthMiddleware), func(m *mvc.Application) {
		...
		m.Party("/ticket-reply").Handle(new(console.TicketReplyController))
