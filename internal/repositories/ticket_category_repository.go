...
func (r *ticketCategoryRepository) FindBySql(db *gorm.DB, sqlStr string, paramArr ...interface{}) (list []models.TicketCategory) {
	db.Raw(sqlStr, paramArr...).Scan(&list)
	return
}

func (r *ticketCategoryRepository) CountBySql(db *gorm.DB, sqlStr string, paramArr ...interface{}) (count int64) {
...
func (r *ticketCategoryRepository) Create(db *gorm.DB, t *models.TicketCategory) (err error) {
	err = db.Create(t).Error
	return
}

func (r *ticketCategoryRepository) Update(db *gorm.DB, t *models.TicketCategory) (err error) {
	err = db.Save(t).Error
	return
}

func (r *ticketCategoryRepository) Updates(db *gorm.DB, id int64, columns map[string]interface{}) (err error) {
	err = db.Model(&models.TicketCategory{}).Where("id = ?", id).Updates(columns).Error
	return
}

func (r *ticketCategoryRepository) UpdateColumn(db *gorm.DB, id int64, name string, value interface{}) (err error) {
	err = db.Model(&models.TicketCategory{}).Where("id = ?", id).UpdateColumn(name, value).Error
	return
}
