...
func (s *ticketService) GetTags(ticketID int64) []models.Tag {
	...
	tagOrder := make(map[int64]int, len(relations))
	for i := range relations {
		tagIDs = append(tagIDs, relations[i].TagID)
		tagOrder[relations[i].TagID] = i
	}
	tags := repositories.TagRepository.Find(sqls.DB(), sqls.NewCnd().In("id", tagIDs))
	if len(tags) <= 1 {
		return tags
	}
	ordered := make([]models.Tag, 0, len(tags))
	indexed := make([]*models.Tag, len(relations))
	for i := range tags {
		tag := tags[i]
		if order, ok := tagOrder[tag.ID]; ok {
			indexed[order] = &tag
		}
	}
